...
class mod_forumng_renderer extends plugin_renderer_base {
    ...
    public function render_blog_item_author($discussion, $courseid) {
        $posteranon = $discussion->get_poster_anon();
        $poster = $discussion->get_poster();
        $userimage = $this->user_picture($poster, array('courseid' => $courseid));
        $defaultimage = html_writer::empty_tag('img',
        ...
        $result = "<div class='forumng-startedby cell c1'>";
        $wrapper = html_writer::start_tag('div', array('class' => 'forumng-startedby-wrapper'));
        $user = $discussion->get_forum()->display_user_link($poster);
        $br = html_writer::empty_tag('br', array());
        $moderator = get_string('moderator', 'forumng');
    ...
    public function render_blog_item(mod_forumng_discussion $discussion, $groupid, $last) {
        ...
        $result = "<div id='blogpost_{$discussion->get_id()}' class='forumng-blogpost$classes'>";

        // Post subject
        $subject = "<div class='forumng-subject'>";
        foreach ($icons as $index => $icon) {
            $alt = $alts[$index];
            if ($icon) {
                $url = $this->pix_url($icon[0], $icon[1]);
                $subject .= "<img src='$url' alt='$alt' title='$alt' /> ";
            } else {
                $subject .= "<span class='accesshide'>$alt:</span> ";
            }
        }
        $subject .= "<a href='discuss.php?" .
                $discussion->get_link_params(mod_forumng::PARAM_HTML) . "'>" .
                format_string($discussion->get_subject(true), true, $courseid) . "</a></div>";
	...
	// Post header
        ...
        $postheader = "<div class='forumng-post-header'>";
        $wrapper = html_writer::start_tag('div', array('class' => 'forumng-startedby-wrapper'));
        $user = "by " . $discussion->get_forum()->display_user_link($poster);
        $timestr = mod_forumng_utils::display_date($discussion->get_time_created());
        $br = html_writer::empty_tag('br', array());
        $moderator = get_string('moderator', 'forumng');
        $userpicture = html_writer::tag('div', $userimage,
                array('class' => 'forumng-startedbyimage'));
        $defaultpicture = html_writer::tag('div', $defaultimage,
                array('class' => 'forumng-startedbyimage'));
        $userlink = html_writer::tag('div', $user ,
                array('class' => 'forumng-startedbyuser')) . ' - ' . $timestr;
        $moderated = html_writer::tag('div', $moderator,
                array('class' => 'forumng-moderator'));
        $endwrapper = html_writer::end_tag('div');
        if ($posteranon == mod_forumng::ASMODERATOR_IDENTIFY) {
            $startedby = $userpicture . $wrapper . $subject . $userlink . $moderated . $endwrapper;
        } else if ($posteranon == mod_forumng::ASMODERATOR_ANON) {
            if ($discussion->get_forum()->can_post_anonymously()) {
                $startedby = $userpicture . $wrapper . $subject . $userlink .  $moderated . $endwrapper;
            } else {
                $startedby = $defaultpicture . $wrapper . $subject .$moderator . $endwrapper;
            }
        } else {
            $startedby = $userpicture . $wrapper .$subject . $userlink . $endwrapper;
        }
        $postheader .= $startedby . "</div>";

	$result .= $postheader;

	// Get post content
	$content = $discussion->get_root_post()->get_formatted_message();

	// Show post
	$result .= "<div class='forumng_post_content'>" . $content . "</div>";

	// Show any attachments
	$attachments = $discussion->get_root_post()->get_attachment_names();
        if (count($attachments)) {
                    ...
                    $result .= '<span class="forumng_post_attachment_label">' . $attachmentlabel .
                ...
                foreach ($attachments as $attachment) {
                    ...
                    $result .= '<li><a href="' . $discussion->get_root_post()->get_attachment_url($attachment) . '">' .
        ...
        $linkparams = $discussion->get_forum()->get_link_params(mod_forumng::PARAM_HTML);
        if ($tags) {
            $taglinks = "<div class='forumng_post_tags'>Tags: ";
            // Unlist them.
            foreach ($tags as $key => $value) {
                $taglinks .= "<a href='view.php?$linkparams&tag=$key'>$value</a>,&nbsp;";
            }
	...
	$result .= $taglinks;
        ...
        // Group
        if ($showgroups) {
            $result .= '<div class="forumng_post_group">'
                . ($discussion->get_group_name()) . '</div>';
        }

        // Number of comments
        if ($discussion->get_num_replies() == 0) {
		$comments = $discussion->get_num_replies();
	} else {
		$comments = "<a href='discuss.php?" .
               	$discussion->get_link_params(mod_forumng::PARAM_HTML) . "'>" .
               	$discussion->get_num_replies() . "</a>";
	}

	$result .= '<div class="forumng_post_comments">Comments: '
            . $comments . '</div>';

	// Comment button
	$result .= '<a class="forumng_comment_btn btn btn-default" href="editpost.php?replyto=' . $discussion->get_root_post()->get_id().'"> Comment</a>';
